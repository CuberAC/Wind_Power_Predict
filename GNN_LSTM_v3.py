...
class GATLayer(nn.Module):
    """
    图注意力层，支持 4D 输入: [Batch, Seq_Len, Nodes, Features]
    """
    ...
    def forward(self, x, adj=None):
        ...
        # 2) 节点对拼接特征
        h_i = h.unsqueeze(3).repeat(1, 1, 1, N, 1)  # [B, S, N, N, F_out]
        h_j = h.unsqueeze(2).repeat(1, 1, N, 1, 1)  # [B, S, N, N, F_out]
        a_input = torch.cat([h_i, h_j], dim=-1)  # [B, S, N, N, 2*F_out]

        # 3) 注意力打分
        e = F.leaky_relu(torch.matmul(a_input, self.a).squeeze(-1), negative_slope=self.alpha)  # [B, S, N, N]
        ...
        # 5) 邻居聚合
        output = torch.matmul(attention, h)  # [B, S, N, F_out]
        return output
...
class WindGNNLSTM(nn.Module):
    ...
    def forward(self, x, adj):
        ...
        x_gat = self.gat2(x_gat, adj)  # (B, 48, 10, gnn_dim)
        
        # 3. 残差拼接：保留原始输入信息
        x_fused = torch.cat([x_gat, x], dim=-1) # (B, 48, 10, 43)
...
def parse_args():
    parser = argparse.ArgumentParser(description='GAT-LSTM v3 训练脚本')
...
def main():
    ...
    exp_dir = os.path.join(args.log_root, f'gat_lstm_v3_{timestamp}')
